...
function Dashboard({ user, token }) {
  ...
  useEffect(() => {
    async function fetchMetrics() {
      ...
      try {
        console.log("Using Firebase token, making API call...");
        const res = await axios.get(
          `${import.meta.env.VITE_BACKEND_URL}/api/metrics`,
          {
  ...
  }, [dateRange, user, token, selectedStore]);

  const pieData = useMemo(() => {
    try {
      return metrics.topCustomers.map((c) => ({
        name: c.name,
  ...
  console.log(
    "Dashboard render - showStoreSelector:",
    showStoreSelector,
    "selectedStore:",
    selectedStore,
    "user:",
  ...
  return (
    <Box>
      ...
      >
        ...
        <Button variant="outlined" onClick={() => setShowStoreSelector(true)}>
          Switch Store
        </Button>
      </Box>
        ...
        <Grid item xs={12} md={3}>
          <Paper elevation={1} sx={{ p: 2 }}>
            ...
            <Typography variant="h4">
              ₹{Number(metrics.revenue).toLocaleString("en-IN")}
            </Typography>
          </Paper>
        </Grid>
        <Grid item xs={12} md={3}>
          <Paper elevation={1} sx={{ p: 2 }}>
            ...
            <Typography variant="h4">
              ₹{Number(metrics.aov).toLocaleString("en-IN", {
          ...
          <Paper elevation={1} sx={{ p: 2, height: 360 }}>
            ...
            >
              <Box>
                ...
                <Typography variant="h4">
                  {(Number(metrics.repeatRate) * 100).toFixed(0)}%
                </Typography>
              </Box>
